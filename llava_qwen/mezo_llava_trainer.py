...
from accelerate.data_loader import SeedableRandomSampler, skip_first_batches
...
from transformers.trainer import XLA_FSDPV2_MIN_VERSION
...
from utils import report_memory, plot_graphs_based_on_log_history
...
class MeZOTrainingArguments(TrainingArguments):
    ...
    zo_eps_schedule: str = field(default="linear", metadata={"help": "Schedule for epsilon in MeZO. (`linear`, `constant`)."})
...
class MeZOLLaVATrainer(LLaVATrainer):
    def __init__(self, **kwargs):
        ...
        if hasattr(args, "trainer_mode"):
            self.trainer_mode = args.trainer_mode
        else:
            self.trainer_mode = "regular"

        if hasattr(args, "zo_eps"):
            self.zo_eps = args.zo_eps
        else:
            self.zo_eps = 1e-3

        if hasattr(args, "zo_num_directions"):
            self.zo_num_directions = args.zo_num_directions
        else:
            self.zo_num_directions = 1

        if hasattr(args, "zo_eps_schedule"):
            self.zo_eps_schedule = args.zo_eps_schedule
        else:
            self.zo_eps_schedule = "linear"
        ...
        if self.trainer_mode == "zo":
            ...
            if self.args.gradient_accumulation_steps != 1:
                print("Warning: MeZO is not compatible with gradient accumulation. Setting gradient_accumulation_steps to 1.")
    ...
    def zo_perturb_parameters(self, scaling_factor=1.0):
        ...
        Args:
            ...
            scaling_factor (float): Scaling factor for perturbation.
        """
    ...
    def zo_step(self, model, inputs):
        ...
        Returns:
            float: Averaged estimated gradient.
        """
        zo_num_directions = self.zo_num_directions
        projected_grads = []

        for _ in range(zo_num_directions):
            ...
            grad_estimate = ((loss_plus - loss_minus) / (2 * self.get_current_zo_eps(self.state.global_step, self.state.max_steps))).item()
            projected_grads.append(grad_estimate)

        self.projected_grad = torch.tensor(projected_grads).mean().item()
    ...
    def zo_update(self, learning_rate):
        ...
        """
        torch.manual_seed(self.zo_random_seed)  # Ensure same perturbation

        print(f"Making MeZO update with grad: {self.projected_grad} with learning rate: {learning_rate}")

        for param in self.trainable_params:
            z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.device, dtype=param.dtype)
            param.data -= learning_rate * (self.projected_grad * z + self.args.weight_decay * param.data)

    def get_current_zo_eps(self, step, num_steps):
        """
        Linearly schedule epsilon for MeZO or keep it constant.
        ...
        Returns:
            float: Scheduled epsilon.
        """
        if self.zo_eps_schedule == "linear":
            return self.zo_eps * (1 - step / num_steps)
